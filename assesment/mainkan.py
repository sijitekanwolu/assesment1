...
def tampilkan_data(sewa_list):
    ...
    for idx, sewa in enumerate(sewa_list, start=1):
        ...
        print(f"{idx}. {sewa.nama_penyewa} | {sewa.no_telp} | {sewa.tanggal_sewa} | {sewa.jam_mulai}-{sewa.jam_selesai} | Durasi: {durasi:.2f} jam | Biaya: Rp{biaya:,.0f}")
...
def tambah_data(sewa_list):
    ...
    sewa = Sewa(nama, telp, tanggal, mulai, selesai)
...
def edit_data(sewa_list):
    ...
    if 0 <= idx < len(sewa_list):
        ...
        sewa_list[idx] = Sewa(nama, telp, tanggal, mulai, selesai)
        file_manager.save_data(sewa_list)
